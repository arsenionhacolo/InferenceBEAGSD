#' @title Response rate to attain a specified p-value (using Method 3 of Nhacolo and Brannath, 2018).
#' @description \code{pipv3} finds the response probability under the null hypotheisis that,
#' given the observed data, would yield a desired overall p-value.
#' @details The p-value is obtained using the Method 3, one of the four methods proposed by
...
#' @seealso  \code{\link{pipv1}}, \code{\link{pipv2}}, \code{\link{pipv2v2}}, \code{\link{aop3e}}.
#' @export
#' @author Arsenio Nhacolo
pipv3 <- function(dsgn, x1o, xo, pv){
  if (x1o == 0) return(NA)
  pi <- -1; pit <- -0.01
  pitv <- c(); cpvv <- c()
  while (pi != pit){
    ...
    cpv <- aop3e(dsgn=dsgn, x1o=x1o, xo=xo, newpi0=pit)
    pitv <- c(pitv, pit)
    cpvv <- c(cpvv, cpv)
    if (cpv > pv){
      pi <-  pit
