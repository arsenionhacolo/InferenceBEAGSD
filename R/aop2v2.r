#' @title Overall p-value (Method 2v2 of Nhacolo and Brannath, 2018).
#' @description \code{aop2v2} calculates the overall p-value for adaptive two-stage designs
#' with binary endpoint using the Method 2v2 (see Nhacolo and Brannath, 2018).
...
#' @seealso \code{\link{aop1}}, \code{\link{aop2}}
#' @export
#' @author Arsenio Nhacolo
aop2v2 <- function(dsgn, x1o, xo, verbose = TRUE){
  stopifnot(xo >= x1o)
  l1 <- dsgn$l1[1]
  u1 <- dsgn$u1[1]
  n1 <- dsgn$n1[1]
  ...
  if (x1o<=l1 | x1o>=u1){
    ...
    pvalue <- 1-pbinom(u1-1,n1,pi0)+
      sum(dbinom(d$x1,n1,pi0)*(1-pbinom(qbinom(1-DDo,d$n2,pi0),d$n2,pi0)))
  }
  if (verbose){
